...
ckpt_dir, figs_dir = dir_create('dp_only_2')
...
for i in range(0, 102):
    ...
    if len(loss_per_epoch) <= 1:
        train_network([0.5])
        print('using HTM1C! dp = 0.5 and {"learning_rate": 1e-4,"momentum": 0.9}')
        
    else:
        train_network([model_dropout_rates[-1]])
        print('Dropout = ', model_dropout_rates[-1])#, lr_rates[-1])
        
    print('model_dropout_rates, loss_per_epoch, ', model_dropout_rates, loss_per_epoch)#, lr_rates)
